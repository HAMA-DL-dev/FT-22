...
#define serial_point 7 // We use 7S 10P, 7 Serial and 10 parallel
...
const uint16_t OV_THRESHOLD = 41500; //!< Over voltage threshold ADC Code. LSB = 0.0001 ---(4.1V)
const uint16_t UV_THRESHOLD = 29000; //!< Under voltage threshold ADC Code. LSB = 0.0001 ---(3V)
...
{
  double total_v = 0;
  double percent_of_v = 0;
  
  for (int current_ic = 0 ; current_ic < TOTAL_IC; current_ic++)
  {
     Serial.print("[");                                                        // checker
     Serial.print(current_ic + 1);                                             // checker
     Serial.print("]");                                                        // checker
     Serial.print(":");                                                        // checker
    if (datalog_en == 0) {
      ...
      for (int i = 0; i < BMS_IC[current_ic].ic_reg.cell_channels; i++)
      {
        ...
        // percent_of_v = (double)(total_v/limit_v)*100;
        percent_of_v = (double)(total_v - 78.4)*2.55; 
        
        // Added SDC logic code // 
        bool is_voltage = (BMS_IC[current_ic].cells.c_codes[i]>100);  
        if(is_voltage){
        ...
        /*
        total_v += (double)(BMS_IC[current_ic].cells.c_codes[i] * 0.0001);
        if(BMS_IC[current_ic].cells.c_codes[i] > UV_THRESHOLD && BMS_IC[current_ic].cells.c_codes[i] < 65535)//
        {
          Serial.print("[");                                                     // 배터리 팩 검차 마무리 과정에서 수정가능하면 주석 처리 
          Serial.print((double)(BMS_IC[current_ic].cells.c_codes[i] ));  // 배터리 팩 검차 마무리 과정에서 수정가능하면 주석 처리 
          Serial.print("]");                                                     // 배터리 팩 검차 마무리 과정에서 수정가능하면 주석 처리 
        }
        else{}
        ...
        if (StateOfCell){
          digitalWrite(SDCout, LOW);
          Serial.println("SDC signal to STM");
          }
        */
       }
       }
...
//    Serial.print(total_v);                                                      // UART transmit total voltage to STM board
     Serial.println(); 
     Serial.println();                                                            
}

/* STM 보드에 전체 전압 값을 전달하기 위한 코드
   String SerialData="";   
   SerialData = String(total_v,2);                                             
